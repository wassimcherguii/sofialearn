<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}" class="h-full {{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>{{ app()->getLocale() === 'ar' ? 'لوحة التحكم الإدارية 2 - سوفيا ليرن' : 'Admin Dashboard 2 - SofiaLearn' }}</title>
    <meta name="description" content="{{ app()->getLocale() === 'ar' ? 'لوحة تحكم إدارية محسنة مع دعم شامل للغة العربية واتجاه النص من اليمين إلى اليسار' : 'Enhanced admin dashboard with comprehensive RTL support for SofiaLearn e-learning platform' }}">
    ...
    <style>
        ...
        /* Flowbite RTL Sidebar positioning using logical properties */
        ...
        /* Flowbite RTL Sidebar border using logical properties */
        html[dir="rtl"] #sidebar {
            border-inline-start: 1px solid #e5e7eb !important;
            border-inline-end: none !important;
        }
        
        /* Flowbite RTL Top navbar */
        html[dir="rtl"] header {
            flex-direction: row-reverse !important;
        }
        
        /* Flowbite RTL User actions */
        html[dir="rtl"] .user-actions {
            flex-direction: row-reverse !important;
        }
        
        /* Flowbite RTL Text alignment */
        html[dir="rtl"] .text-left {
            text-align: right !important;
        }
        
        /* Flowbite RTL Navigation using logical properties */
        ...
        /* Flowbite RTL Tooltips using logical properties */
        html[dir="rtl"] .sidebar-collapsed-tooltip {
        ...
        /* Flowbite RTL Stats cards */
        html[dir="rtl"] .stats-card .flex {
            flex-direction: row-reverse !important;
        }
        
        /* Flowbite RTL Quick actions using logical properties */
        html[dir="rtl"] .quick-action {
            flex-direction: row-reverse !important;
        }
        
        html[dir="rtl"] .quick-action .icon-container {
            margin-inline-end: 0 !important;
            margin-inline-start: 1rem !important;
        }
        
        /* Flowbite RTL Activity items using logical properties */
        html[dir="rtl"] .activity-item {
            flex-direction: row-reverse !important;
        }
        
        html[dir="rtl"] .activity-item .activity-dot {
            margin-inline-end: 0 !important;
            margin-inline-start: 0.75rem !important;
        }
        
        /* Flowbite RTL Dropdown positioning using logical properties */
        html[dir="rtl"] .dropdown-menu {
            inset-inline-end: 0 !important;
            inset-inline-start: auto !important;
        }
        
        /* Flowbite RTL Form elements */
        ...
        /* Flowbite RTL Mobile sidebar using logical properties */
        ...
        /* Flowbite RTL Language dropdown */
        html[dir="rtl"] .language-dropdown {
            flex-direction: row-reverse !important;
        }
        
        /* Debug indicators removed for production */
    </style>
</head>
<body class="h-full paper-texture">
    <div class="flex flex-col h-full">
        <!-- Top Navbar - Full Width with Enhanced RTL Support -->
        <header class="bg-white shadow-sm border-b border-gray-100 px-4 py-3 flex items-center justify-between" role="banner" aria-label="{{ app()->getLocale() === 'ar' ? 'شريط التنقل الرئيسي' : 'Main navigation bar' }}">
            <!-- Logo Box (Sidebar Width) - RTL Aware -->
            <div class="flex items-center w-64 logo-box" role="banner" aria-label="{{ app()->getLocale() === 'ar' ? 'شعار الموقع' : 'Site logo' }}">
                <!-- Desktop toggle button -->
                <button id="toggleSidebar" class="hidden md:block p-2 rounded-lg hover:bg-gray-100 me-3 burger-button">
                ...
                <!-- Mobile menu button -->
                <button id="openSidebar" class="md:hidden p-2 rounded-lg hover:bg-gray-100 me-2 burger-button">
                    <svg class="w-6 h-6 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
                    </svg>
                </button>
                ...
                <div class="flex items-center space-x-3 logo-section">
                    ...
                    <div class="sidebar-logo-text transition-all duration-300">
                        ...
                        <p class="text-xs text-gray-500">Admin Panel v2</p>
                    </div>
                </div>
            </div>
    
            <!-- User Info & Actions - RTL Aware -->
            <div class="flex items-center space-x-4 user-actions">
                ...
                <!-- Enhanced Language Dropdown with RTL Support -->
                <div class="language-dropdown">
                    ...
                    <!-- Dropdown menu with RTL positioning -->
                    <div id="languageDropdownMenu" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dropdown-menu">
        ...
        <div class="flex flex-1">
            <!-- Enhanced Sidebar with RTL Support -->
            <div id="sidebar" class="w-64 bg-white shadow-lg {{ app()->getLocale() === 'ar' ? 'border-l' : 'border-r' }} border-gray-100 sidebar-transition md:translate-x-0 sidebar-hidden fixed md:relative top-0 {{ app()->getLocale() === 'ar' ? 'right-0' : 'left-0' }} h-full z-40" data-collapsed="false" role="navigation" aria-label="{{ app()->getLocale() === 'ar' ? 'القائمة الجانبية' : 'Sidebar navigation' }}" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
                <div class="flex flex-col h-full">
                    <!-- Navigation with RTL Support -->
                    <nav class="flex-1 px-4 py-6 space-y-2 sidebar-nav" role="navigation" aria-label="{{ app()->getLocale() === 'ar' ? 'قائمة التنقل الرئيسية' : 'Main navigation menu' }}">
                        <a href="#" class="flex items-center px-4 py-3 text-emerald-600 bg-emerald-50 rounded-xl font-medium group">
                            ...
                            <div class="sidebar-collapsed-tooltip absolute start-full ms-2 px-2 py-1 bg-gray-800 text-white text-sm rounded opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap z-50">
            ...
            <!-- Main Content with Enhanced RTL Support -->
            <div id="mainContent" class="flex-1 flex flex-col sidebar-transition">
                <!-- Main Content Area -->
                <main class="flex-1 p-4 md:p-6 text-left">
                    <!-- Enhanced Stats Cards with RTL Support -->
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                        <div class="bg-white rounded-2xl shadow-lg shadow-gray-100/50 border border-gray-100 p-6 hover:shadow-xl transition-all duration-300 stats-card">
                    ...
                    <!-- Content Grid with RTL Support -->
                    <div class="grid lg:grid-cols-3 gap-8">
                        <!-- Quick Actions with RTL Support -->
                        <div class="lg:col-span-2">
                            <div class="bg-white rounded-2xl shadow-lg shadow-gray-100/50 border border-gray-100 p-8">
                                ...
                                <div class="grid md:grid-cols-2 gap-4">
                                    <a href="{{ route('courses.index') }}" class="group flex items-center p-6 bg-emerald-50 hover:bg-emerald-100 rounded-xl transition-all duration-300 quick-action">
                                        <div class="w-12 h-12 bg-emerald-500 rounded-xl flex items-center justify-center icon-container group-hover:scale-110 transition-transform">
                                    ...
                                    <a href="#" class="group flex items-center p-6 bg-red-50 hover:bg-red-100 rounded-xl transition-all duration-300 quick-action">
                                        <div class="w-12 h-12 bg-red-500 rounded-xl flex items-center justify-center icon-container group-hover:scale-110 transition-transform">
                                            <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z"/>
                                            </svg>
                                        </div>
                                    ...
                                    <a href="#" class="group flex items-center p-6 bg-amber-50 hover:bg-amber-100 rounded-xl transition-all duration-300 quick-action">
                                        <div class="w-12 h-12 bg-amber-500 rounded-xl flex items-center justify-center icon-container group-hover:scale-110 transition-transform">
                                            <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                                            </svg>
                                        </div>
                                    ...
                                    <a href="#" class="group flex items-center p-6 bg-blue-50 hover:bg-blue-100 rounded-xl transition-all duration-300 quick-action">
                                        <div class="w-12 h-12 bg-blue-500 rounded-xl flex items-center justify-center icon-container group-hover:scale-110 transition-transform">
                                            <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                            </svg>
                        ...
                        <!-- Recent Activity with RTL Support -->
                            ...
                            <div class="bg-white rounded-2xl shadow-lg shadow-gray-100/50 border border-gray-100 p-8">
                                ...
                                <div class="space-y-4">
                                    <div class="flex items-start space-x-3 activity-item">
                                        <div class="w-2 h-2 bg-emerald-500 rounded-full mt-2 activity-dot"></div>
                                        <div>
                                    ...
                                    <div class="flex items-start space-x-3 activity-item">
                                        <div class="w-2 h-2 bg-blue-500 rounded-full mt-2 activity-dot"></div>
                                        <div>
                                    ...
                                    <div class="flex items-start space-x-3 activity-item">
                                        <div class="w-2 h-2 bg-amber-500 rounded-full mt-2 activity-dot"></div>
                                        <div>
                                    ...
                                    <div class="flex items-start space-x-3 activity-item">
                                        <div class="w-2 h-2 bg-red-500 rounded-full mt-2 activity-dot"></div>
                                        <div>
    ...
    <script>
        // Enhanced sidebar toggle functionality with RTL support
        ...
        // Enhanced Flowbite language dropdown selection with RTL support
